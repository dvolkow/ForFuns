...
MU = 1.0                                      # --- Заданный параметр распределения
SIGMA = 5.0                                   # --- Заданный параметр распределения
M = MU                                      # --- Теоретическое матожидание
D = SIGMA ** 2                              # --- Теоретическая дисперсия
...
for i in range(COUNT_ROUNDS):               # --- в этом цикле получаем выборки
    ...
    random_array = np.array([np.random.normal(MU, SIGMA) for i in range(COUNT_ELEMENT)])
...
plt.plot([M + 3.0 * math.sqrt(D / (i + 1)) for i in range(COUNT_ELEMENT)], 'r--', linewidth=1.5)
plt.plot([M - 3.0 * math.sqrt(D / (i + 1)) for i in range(COUNT_ELEMENT)], 'r--', linewidth=1.5)
